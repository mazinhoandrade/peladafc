...
@section('content')
  ...
  <tr>
                    <td>{{$baba->id}}</td>
                    <td>{{date('d/m/Y', strtotime($baba->created_at))}}</td>
                    <td>{{$baba->descricao}}</td>
                    <td>
                        <a href="{{ route('baba.edit', $baba->id)}}" class="btn btn-sm"><img src="http://127.0.0.1:8000/storage/media/img/edit.png" width="30px"></a>
  ...
  @endforeach
    ...
    <div class="container-fluid desk">
            {{ $babas->links('pagination::bootstrap-4') }}
    </div>
@endsection
